  ...
  const validateForm = () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (isNaN(lat) || lat < -90 || lat > 90) {
        setStatus({
          type: "danger",
          message: `Vertex ${i + 1}: Invalid latitude (must be -90 to 90)`,
        });
        return false;
      }
      ...
      if (isNaN(lng) || lng < -180 || lng > 180) {
        setStatus({
          type: "danger",
          message: `Vertex ${i + 1}: Invalid longitude (must be -180 to 180)`,
        });
        return false;
      }
    }
        ...
        >
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-4">
              <Form.Label style={{ fontWeight: "bold", color: "#333" }}>Building Name</Form.Label>
              <Form.Control
                type="text"
            ...
            {vertices.map((vertex, index) => (
              <Card key={index} className="mb-3" style={{ border: "2px solid #f0f0f0" }}>
                <Card.Body>
                  <h6 style={{ color: "#c5050c", marginBottom: "15px" }}>Vertex {index + 1}</h6>
